...
(* Local vars table cleared evertime a function is built *)
...
let rec get_pointer_type datatype = match datatype with
    A.Array(t, 0) -> get_llvm_type t
  | A.Array(t, 1) -> L.pointer_type (get_llvm_type t)
  | A.Array(t, i) -> L.pointer_type (get_pointer_type (A.Array(t, (i-1))))
...
(* let translate (globals, functions) =
  ...
  let the_module = L.create_module context "Strux"
  and i32_t  = L.i32_type  context (* num *)

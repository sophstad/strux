...
and translate (globals, functions) =
  ...
  let cQuickfSort_t = L.function_type (L.pointer_type (ltype_of_typ A.Num)) [| L.pointer_type (ltype_of_typ A.Int); i32_t |] in
  let cQuickfSort_f = L.declare_function "cQuickfSort" cQuickfSort_t the_module in
  let cShowfQuickSort_t = L.function_type (L.pointer_type (ltype_of_typ A.Num)) [| L.pointer_type (ltype_of_typ A.Int); i32_t |] in
